...
/// a one way, meaning that u just come to one layer and bind it to the other (i prefer the controller to the viewModel)
...
/// if we used bind from a thread, then we are actually... well, dealing with bits and bytes being modified, so u get a really unexpected results
class Dynamic<T> {
